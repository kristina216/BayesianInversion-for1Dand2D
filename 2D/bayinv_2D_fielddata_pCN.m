% DC 2D problem for Checkerboard with pCN algorithm
% Bayesian Inversion
...
% Declare variables
proposed_log_rhoa = zeros(len_split, 1);
...
for i = 2:n
    ...
    current_log_rhoa = log(1./target_leitf(:,i-1));
    
    % Generating new candidate for random walk
    for k = 1:len_split 
        proposed_log_rhoa(k) = sqrt(1-s^2) * current_log_rhoa(k) ...
            + s*randn(1,1);
    end
    
    % Fixed background resistivity
    proposed_log_rhoa(1) = log(400);
    proposed_log_rhoa(end) = log(400);
    
    % First layer
    proposed_log_rhoa(2) = log(200); % log(100)
    
    % Set up parameter space candidate
    param_prop = (cm == unique(cm).') * proposed_log_rhoa;
    ...
    if hlp_time == 1
        ...
        param_curr = (cm == unique(cm).') * current_log_rhoa;
    ...
    if u <= min(A, 1)
        % Random walk
        target_leitf(:,i) = 1./exp(proposed_log_rhoa); 
        
        % Results
        result.layer(i-1,:) = exp(proposed_log_rhoa);
        ...
        % Random walk
        target_leitf(:,i) = 1./exp(current_log_rhoa);
